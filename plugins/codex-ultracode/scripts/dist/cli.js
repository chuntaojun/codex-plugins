...
function defaultDispatchPlanPath(intent) {
...
export function buildRunFollowUpEvent(runId) {
...
function emitRunFollowUp(runId) {
...
function createCliWorktreePayload(prepared) {
    return {
...
function parseBooleanOption(options, key, defaultValue = false) {
...
function splitShortcutArgs(argv) {
...
export function parseCliArgs(argv) {
    ...
    if (argv[0] === "plan-dynamic") {
        ...
        return {
            ...
            intent,
            params: parseJsonObjectOption(options, "params"),
            outputPlan: options.outputPlan ?? options["output-plan"],
            outputDir: options.outputDir ?? options.output,
            runId: options.runId ?? options["run-id"],
            worktreeDir: options.worktreeDir ?? options["worktree-dir"],
...
export function formatWorkflowStatus(status) {
    ...
    if (status.failedGates.length > 0) {
        ...
        lines.push(...status.failedGates.map((gate) => `- ${gate.stage}: ${gate.command} exited ${gate.exitCode}`));
    }
    return lines.join("\n");
}
export function formatWorkflowTail(tail) {
    if (tail.events.length === 0) {
        return `No events found in ${tail.traceFile}`;
    }
    return tail.events.map((event) => JSON.stringify(event)).join("\n");
}
export function formatWorkflowReport(report) {
...
export function formatNamedWorkflowList(workflows) {
    if (workflows.length === 0) {
        return "No registered Ultracode workflows found.";
    }
    return workflows
        .map((workflow) => workflow.description ? `${workflow.name}: ${workflow.description}` : workflow.name)
        .join("\n");
}
export function formatNamedWorkflowDescription(workflow, readme) {
    const lines = [
...
function formatStagePlan(stagePlan) {
    ...
    return [
        "Stage plan:",
        ...stagePlan.map((stage) => `- ${stage.name} [${stage.mode}] -> ${stage.outputFile} (depends on: ${stage.dependsOn.length > 0 ? stage.dependsOn.join(", ") : "none"})`),
    ];
}
function createCliNamedWorkflowPreview(workflow, options) {
...
export function formatDynamicWorkflowPlan(plan) {
...
function resolveDynamicPlanForRun(input) {
...
function createCliExecutionAfterApproval(nextCommand, workflowKind) {
    return {
        nextCommand,
        confirmationGate: `Only run this command after the user has approved the ${workflowKind} workflow preview.`,
    };
}
function createCliExecution(nextCommand) {
    return {
        nextCommand,
        approvalRequired: false,
    };
}
async function runNamedWorkflowFromCli(input, invocationRoot, options = {}) {
...
export function formatWorkflowStageAction(result) {
    return [
...
function createWorkflowStageActionPayload(result, target) {
    return {
        ...result,
...
export function formatRunList(runs) {
    ...
    return runs
        .map((run) => `${run.runId} ${run.workflow} ${run.status} ${run.source} ${run.updatedAt}\n  ${run.outputDir}`)
        .join("\n");
}
export function formatRunArtifact(artifact) {
    return [
        `Run: ${artifact.runId}`,
...
export function formatPruneRuns(result) {
    const lines = [
        ...result.removed.map((run) => `Removed ${run.runId}: ${run.outputDir}`),
        ...result.removedWorktrees.map((worktree) => `Removed worktree ${worktree.runId}: ${worktree.worktreeRoot}`),
...
export async function main(argv = process.argv.slice(2)) {
    ...
    if (input.command === "plan-dynamic") {
        ...
        if (plan.recommendedAction === "run_named") {
            const workflow = availableWorkflows.find((entry) => entry.name === plan.workflowName) ??
        ...
        const planFile = writeDynamicPlanFile({
            planFile: path.resolve(process.cwd(), input.outputPlan ?? defaultDispatchPlanPath(input.intent)),
            intent: input.intent,
            plan,
            params: input.params,
            execution: {
    ...
    if (input.command === "dispatch") {
        ...
        if (plan.recommendedAction === "run_named") {
            ...
            if (writeStages.length > 0) {
                ...
                input.json
                    ? printJson(payload)
                    : console.log(formatNamedWorkflowConfirmation({
                        workflow,
                        intent: input.intent,
                        reason: plan.reason,
                        nextCommand,
            ...
            const runPayload = await runNamedWorkflowFromCli({
                ...
                registryDir: input.registryDir,
                json: input.json,
            }, invocationRoot, { silent: input.json });
            if (input.json) {
                printJson({
                    dispatch: {
                        action: "ran_named",
        ...
        input.json
            ? printJson(payload)
            : console.log(formatDynamicDispatchConfirmation({
                plan,
                planFile: planFile.planFile,
                nextCommand,
            }));
