...
export function prepareRunWorktree(repoRoot, options = {}) {
    ...
    if (!status.isGitRepository) {
        throw new Error("ultracode run must start inside a git repository so it can create an isolated run worktree.");
    }
    if (status.isSubmodule) {
        throw new Error("ultracode run cannot create a run worktree from a git submodule. Start from the superproject checkout instead.");
    }
    ...
    const branch = `codex/ultracode-${runId}`;
