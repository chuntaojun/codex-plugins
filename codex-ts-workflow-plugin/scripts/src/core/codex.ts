...
export function createCodexJsonlState(): CodexJsonlState {
  return {
    buffer: "",
  };
}
...
export function flushCodexJsonBuffer(
  state: CodexJsonlState,
  onJsonLine: (line: string) => void,
): void {
...
export function buildCodexArgs(options: AgentOptions): string[] {
  const args = [
    "exec",
    options.prompt,
    "--cd",
...
export const agent: AgentRunner = (options) => {
    ...
    child.stdout.on("data", (buf: Buffer) => {
      ...
      if (options.eventFile) {
        consumeCodexJsonChunk(jsonlState, data, (line) => {
          fs.appendFileSync(options.eventFile as string, `${line}\n`);
        });
      }
      emitEvent({
        type: "agent.event",
