...
export type DynamicWorkflowTemplate =
  | "debug-fix"
  | "code-change"
  | "research"
  | "content-deliverable"
...
function scorePhraseMatch(intent: string, phrase: string, weight: number): number {
...
function scoreNamedWorkflow(intent: string, workflow: NamedWorkflowSummary): number {
...
function findMatchingNamedWorkflow(
  intent: string,
  workflows: NamedWorkflowSummary[],
): NamedWorkflowSummary | undefined {
  let best: { workflow: NamedWorkflowSummary; score: number } | undefined;
...
function isCodeChangeIntent(intent: string): boolean {
  return /fix|bug|failing|test|implement|code|change|update|refactor|modify|patch|修复|实现|改代码|修改|重构|测试失败/i.test(intent);
}

function isReviewOnlyIntent(intent: string): boolean {
  return /review|risk|risks|report|audit|assess|assessment|evaluate|evaluation|analyze|analysis|summarize|summary|inspect|investigate|复盘|风险|报告|审阅|评审|分析|总结|检查|调研|研究/i.test(
    intent,
  );
}

function isDebugFixIntent(intent: string): boolean {
  return /debug|failing|failure|failed|ci|test failure|tests? fail|error|exception|regression|root cause|bug|排障|调试|失败|报错|异常|根因|测试失败|CI/i.test(
    intent,
  );
}

function isResearchIntent(intent: string): boolean {
...
function isContentDeliverableIntent(intent: string): boolean {
  return /travel guide|itinerary|trip plan|release notes|changelog|proposal|briefing|playbook|guide|document|email|announcement|攻略|行程|旅行|旅游|发布说明|变更日志|方案|文档|邮件|公告|指南|手册|计划书|提案/i.test(
    intent,
  );
}

function createDebugFixWorkflow(intent: string): DynamicWorkflow {
  return {
...
function createCodeChangeWorkflow(intent: string): DynamicWorkflow {
  return {
    name: slugifyIntent(intent),
...
function createResearchWorkflow(intent: string): DynamicWorkflow {
  return {
    name: slugifyIntent(intent),
...
function createContentDeliverableWorkflow(intent: string): DynamicWorkflow {
  return {
    name: slugifyIntent(intent),
...
function createConstrainedWorkflow(intent: string): DynamicWorkflow {
  return {
    name: slugifyIntent(intent),
...
function createPreview(
  ...
  return {
    ...
    risks,
    confirmationPrompt:
      "Review this preview and the full workflow JSON, then approve execution with approved=true only if the scope is correct.",
  };
}

function dynamicTemplateReason(template: DynamicWorkflowTemplate): string {
  switch (template) {
    case "debug-fix":
...
export function planDynamicWorkflow(options: PlanDynamicWorkflowOptions): DynamicWorkflowPlan {
  ...
  const template: DynamicWorkflowTemplate = isDebugFixIntent(intent)
    ? "debug-fix"
