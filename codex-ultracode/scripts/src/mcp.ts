...
import { planDynamicWorkflow, type DynamicWorkflowPlan } from "./dynamic-planner.js";
import {
  ...
  type ApprovedDynamicPlanFile,
  type WrittenDynamicPlanFile,
} from "./dynamic-plan-file.js";
import {
  createDynamicWorkflowFiles,
  type CreateDynamicWorkflowFilesOptions,
  type CreatedDynamicWorkflowFiles,
...
import {
  ...
  type CreateParamFileOptions,
  type CreatedParamFile,
  type NamedWorkflowSummary,
  type ResolveNamedWorkflowOptions,
} from "./named-workflows.js";
import {
  restartWorkflowStage,
  reworkWorkflowStage,
  type WorkflowStageActionOptions,
  type WorkflowStageActionResult,
  type WorkflowStageReworkOptions,
...
import {
  ...
  pruneRuns,
  resolveRunOutputDir,
  type GetRunArtifactOptions,
  type ListRunsOptions,
  type PruneRunsOptions,
  type ResolveRunOutputDirOptions,
...
type WorkflowInspectionLinks = {
  runId: string;
  outputDir: string;
  approvedPlanFile?: string;
  approvedNamedWorkflowFile?: string;
  stages?: WorkflowStageInspectionLink[];
  status: {
...
type WorkflowStageInspectionLink = {
  stageName: string;
  status?: string;
  attempt?: number;
  artifact?: WorkflowStageArtifactLink;
  restartStage: {
    tool: "ultracode_restart_stage";
    arguments: {
      cwd: string;
...
type WorkflowStageArtifactLink = {
  outputFile?: string;
  resultFile?: string;
  attemptDir?: string;
  stageDir?: string;
...
type McpDependencies = {
  ...
  createDynamicWorkflowFiles?: (
    options: CreateDynamicWorkflowFilesOptions,
  ) => CreatedDynamicWorkflowFiles;
...
function defaultDynamicPlanPath(intent: string): string {
...
function createWorkflowInspectionLinks(options: {
...
}): WorkflowInspectionLinks {
  const stages = (options.stages ?? []).map((stage): WorkflowStageInspectionLink => {
    ...
    }) as WorkflowStageArtifactLink;
    const link: WorkflowStageInspectionLink = {
  ...
  if (options.approvedNamedWorkflowFile) {
    nextActions.push(
      "Use inspection.approvedNamedWorkflowFile as the audit record for the approved named workflow.",
    );
  }
  const inspection: WorkflowInspectionLinks = {
    runId: options.runId,
...
type NamedWorkflowFollowUp = "execution" | "executionAfterApproval" | "missingCwd";

function createNamedWorkflowPreview(
  workflow: NamedWorkflowSummary,
  options: { followUp: NamedWorkflowFollowUp },
): Record<string, unknown> {
...
function createStageActionPayload(
...
const RUN_WORKFLOW_TOOL = {
  name: "ultracode_run",
  description:
    "Run a JSON-defined Codex workflow through ultracode's runtime in an isolated git worktree.",
  inputSchema: {
    type: "object",
    properties: {
      cwd: {
      ...
      outputDir: {
        type: "string",
        description:
          "Optional artifact directory, resolved inside the prepared run worktree. Defaults to .ultracode/runs/<run-id>.",
      },
      runId: {
        type: "string",
...
const RUN_NAMED_WORKFLOW_TOOL = {
  ...
  description:
    "Run a named Ultracode workflow by workflow name and task intent. Use this instead of file paths when a registered workflow fits the user request.",
  inputSchema: {
    type: "object",
    properties: {
      cwd: {
      ...
      approved: {
        type: "boolean",
        description:
          "Must be true when running a write-capable named workflow after the user approved its preview. Read-only named workflows do not require it.",
      },
    },
...
const DISPATCH_INTENT_TOOL = {
  ...
  description:
    "Handle an open-ended Ultracode task intent in one call. Runs a matching read-only named workflow immediately, returns a confirmation preview for a write-capable named workflow, or returns a dynamic workflow preview when no registered workflow fits.",
  inputSchema: {
    type: "object",
    properties: {
      cwd: {
      ...
      params: {
        type: "object",
        description:
          "Optional structured parameters to merge into a named workflow parameter template or dynamic workflow params.",
      },
      outputDir: {
        type: "string",
        description:
          "Optional artifact directory when a named workflow is run. Resolved inside the prepared run worktree.",
      },
      outputPlan: {
        type: "string",
        description:
          "Optional deterministic plan artifact path when the request needs a dynamic workflow preview.",
      },
      runId: {
        type: "string",
...
const PLAN_DYNAMIC_WORKFLOW_TOOL = {
  ...
  description:
    "Plan how Ultracode should handle a task intent. Recommends a named workflow when possible, otherwise returns a constrained dynamic workflow preview without executing it.",
  inputSchema: {
    type: "object",
    properties: {
      intent: {
      ...
      cwd: {
        type: "string",
        description:
          "Optional repository checkout directory. Pass the current Codex workspace root so named workflow recommendations and dynamic previews can return ready-to-call execution arguments, and dynamic previews can save default plan artifacts.",
      },
      outputPlan: {
        type: "string",
        description:
          "Optional path for a reusable plan artifact. Relative paths require cwd. If omitted, Ultracode writes a default artifact only for review_dynamic previews when cwd is provided, not for run_named recommendations.",
      },
      registryDir: {
        type: "string",
        description: "Optional named workflow registry directory for development or tests.",
      ...
      params: {
        type: "object",
        description:
          "Optional structured parameters to include when a named workflow is recommended or when a dynamic workflow is approved.",
      },
      outputDir: {
        type: "string",
        description:
          "Optional artifact directory to preserve in approved dynamic execution arguments.",
      },
      runId: {
        type: "string",
      ...
      worktreeDir: {
        type: "string",
        description:
          "Optional explicit run worktree directory to preserve in approved dynamic execution arguments.",
      },
    },
    required: ["intent"],
    additionalProperties: false,
...
const RUN_DYNAMIC_WORKFLOW_TOOL = {
  ...
  description:
    "Run an approved dynamic Ultracode workflow preview from a reviewed plan file. Only call this after the user has reviewed and approved the preview returned by ultracode_plan_dynamic or ultracode_dispatch. The run writes approved-plan.json with the effective params and execution context used for this run.",
  inputSchema: {
    type: "object",
    properties: {
      cwd: {
      ...
      planFile: {
        type: "string",
        description:
          "Dynamic plan artifact returned by ultracode_plan_dynamic or ultracode_dispatch. Workflow, intent, and saved preview context are read from this reviewed plan file.",
      },
      params: {
        type: "object",
        description:
          "Optional structured parameters to include in param.dynamic.json. Saved plan-file params are used as the base when present; explicit object fields deep-merge over them, while arrays and non-objects override.",
      },
      outputDir: {
        type: "string",
        description:
          "Optional artifact directory, resolved inside the prepared run worktree. If omitted, saved plan-file outputDir is reused when present; otherwise defaults to .ultracode/runs/<run-id>.",
      },
      runId: {
        type: "string",
        description:
          "Optional stable run id used for the branch, worktree directory, and default outputDir. If omitted, saved plan-file runId is reused when present.",
      },
      worktreeDir: {
        type: "string",
        description:
          "Optional explicit run worktree directory. If omitted, saved plan-file worktreeDir is reused when present.",
      },
    },
...
const RESTART_STAGE_TOOL = {
  ...
  description:
    "Restart a workflow stage by creating the next attempt with the same input as the latest attempt. Accepts an artifact directory or run id.",
  inputSchema: {
    type: "object",
    properties: {
      outputDir: {
...
const REWORK_STAGE_TOOL = {
  ...
  description:
    "Rework a workflow stage by creating the next attempt with structured feedback added to input.rework. Accepts an artifact directory or run id.",
  inputSchema: {
    type: "object",
    properties: {
      outputDir: {
...
const LIST_RUNS_TOOL = {
  ...
  description:
    "List recent Ultracode runs from the current checkout and sibling run worktrees.",
  inputSchema: {
    type: "object",
    properties: {
      cwd: {
...
const PRUNE_RUNS_TOOL = {
  ...
  description:
    "Remove selected Ultracode run artifact directories by run id. By default it does not remove sibling run worktrees; set removeWorktrees=true to remove them explicitly.",
  inputSchema: {
    type: "object",
    properties: {
      cwd: {
      ...
      removeWorktrees: {
        type: "boolean",
        description:
          "When true, also remove matching sibling <repo>.worktrees/<run-id> directories for worktree-backed runs.",
      },
    },
    required: ["cwd", "runIds"],
    additionalProperties: false,
...
function optionalParams(input: Record<string, unknown>): JsonRecord | undefined {
...
function requireRecord(input: Record<string, unknown>, key: string): JsonRecord {
  const value = input[key];
...
function buildRunNamedWorkflowInput(input: Record<string, unknown>): RunNamedWorkflowInput {
  return {
    cwd: path.resolve(requireString(input, "cwd")),
...
function buildRunDynamicWorkflowInput(input: Record<string, unknown>): RunDynamicWorkflowInput {
...
function buildDispatchIntentInput(input: Record<string, unknown>): DispatchIntentInput {
  return {
    cwd: path.resolve(requireString(input, "cwd")),
...
function resolveDynamicPlanForMcpRun(options: {
  ...
  return {
    ...
    workflow: planFile.plan.workflow as unknown as JsonRecord,
    planFile,
  };
}
...
async function handleRunWorkflowTool(
  ...
  try {
    ...
    const outputDir = path.resolve(
      ...
      input.outputDir ?? `.ultracode/runs/${runId}`,
    );
...
async function handleRunNamedWorkflowTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): Promise<ToolResult> {
  let input: RunNamedWorkflowInput;
  try {
  ...
  const writeApprovedNamedWorkflow =
    dependencies.writeApprovedNamedWorkflowFile ?? writeApprovedNamedWorkflowFile;
...
function handleListWorkflowsTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): ToolResult {
  try {
...
function handleDescribeWorkflowTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): ToolResult {
  try {
...
async function handleDispatchIntentTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): Promise<ToolResult> {
  let input: DispatchIntentInput;
  try {
...
function handlePlanDynamicWorkflowTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): ToolResult {
  try {
    ...
    const plan: DynamicWorkflowPlan = (dependencies.planDynamicWorkflow ?? planDynamicWorkflow)({
      intent,
      availableWorkflows,
    });
    const matchedNamedWorkflow =
      plan.recommendedAction === "run_named"
        ? availableWorkflows.find((workflow) => workflow.name === plan.workflowName)
        : undefined;
    const matchedNamedWorkflowNeedsConfirmation =
      (matchedNamedWorkflow?.writeStages ?? []).length > 0;
    const execution =
      cwd && plan.recommendedAction === "run_named" && !matchedNamedWorkflowNeedsConfirmation
    ...
    const executionAfterApproval =
      cwd && plan.recommendedAction === "run_named" && matchedNamedWorkflowNeedsConfirmation
        ...
        : undefined;
    const namedWorkflowFollowUp: NamedWorkflowFollowUp | undefined = matchedNamedWorkflow
      ? execution
        ? "execution"
    ...
    if (!resolvedOutputPlan) {
      const payload = {
        ...
        preview:
          matchedNamedWorkflow && namedWorkflowFollowUp
    ...
    const dynamicExecutionAfterApproval =
      cwd && plan.recommendedAction === "review_dynamic"
...
async function handleRunDynamicWorkflowTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): Promise<ToolResult> {
  let input: RunDynamicWorkflowInput;
  try {
  ...
  if (!input.approved) {
    return contentResult(
      "Dynamic workflow execution requires approved=true.",
      { error: "Dynamic workflow execution requires approved=true." },
      true,
    );
  }
  ...
  const createDynamicFiles =
    dependencies.createDynamicWorkflowFiles ?? createDynamicWorkflowFiles;
  ...
  try {
    ...
    const generated = createDynamicFiles({
      ...
      workflow: resolvedPlan.workflow as unknown as CreateDynamicWorkflowFilesOptions["workflow"],
      params: effectiveParams,
    });
    let approvedPlan: ApprovedDynamicPlanFile | undefined;
    ...
    const payload = {
      ...
      plan: resolvedPlan.planFile
        ? {
            ...
            preview:
              resolvedPlan.planFile.plan.recommendedAction === "review_dynamic"
...
async function handleRestartStageTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): Promise<ToolResult> {
  try {
...
async function handleReworkStageTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): Promise<ToolResult> {
  try {
...
function handleRunArtifactTool(
  rawInput: Record<string, unknown>,
  dependencies: McpDependencies,
): ToolResult {
  try {
...
async function handleToolCall(
  ...
  if (name === RUN_NAMED_WORKFLOW_TOOL.name) {
    return handleRunNamedWorkflowTool(input as Record<string, unknown>, dependencies);
  }
  if (name === LIST_WORKFLOWS_TOOL.name) {
    return handleListWorkflowsTool(input as Record<string, unknown>, dependencies);
  }
  if (name === DESCRIBE_WORKFLOW_TOOL.name) {
    return handleDescribeWorkflowTool(input as Record<string, unknown>, dependencies);
  }
  if (name === DISPATCH_INTENT_TOOL.name) {
    return handleDispatchIntentTool(input as Record<string, unknown>, dependencies);
  }
  if (name === PLAN_DYNAMIC_WORKFLOW_TOOL.name) {
    return handlePlanDynamicWorkflowTool(input as Record<string, unknown>, dependencies);
  }
  if (name === RUN_DYNAMIC_WORKFLOW_TOOL.name) {
    return handleRunDynamicWorkflowTool(input as Record<string, unknown>, dependencies);
  }
  if (name === RESTART_STAGE_TOOL.name) {
    return handleRestartStageTool(input as Record<string, unknown>, dependencies);
  }
  if (name === REWORK_STAGE_TOOL.name) {
    return handleReworkStageTool(input as Record<string, unknown>, dependencies);
  }
  if (name === LIST_RUNS_TOOL.name) {
    return handleListRunsTool(input as Record<string, unknown>, dependencies);
  }
  if (name === RUN_ARTIFACT_TOOL.name) {
    return handleRunArtifactTool(input as Record<string, unknown>, dependencies);
  }
  if (name === PRUNE_RUNS_TOOL.name) {
    return handlePruneRunsTool(input as Record<string, unknown>, dependencies);
  }
...
function resolveInspectOutputDir(
  input: Record<string, unknown>,
  dependencies: McpDependencies,
): string {
