...
function isSafeRelativeOutputFile(file: string): boolean {
...
export function validateDynamicWorkflow(workflow: unknown): DynamicWorkflowValidation {
  const errors: string[] = [];
  ...
  const dependencyValidation = validateWorkflowDependencies(
    workflow.stages.filter(isRecord).map((stage) => ({
      name: typeof stage.name === "string" ? stage.name : "",
      dependsOn: Array.isArray(stage.dependsOn)
        ? stage.dependsOn.filter((dependency): dependency is string => typeof dependency === "string")
...
export function createDynamicWorkflowFiles(
