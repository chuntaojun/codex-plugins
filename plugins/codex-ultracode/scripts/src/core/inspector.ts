...
import type { CommandGateResult, StageResult } from "./stage.js";

type WorkflowResult = {
  workflow: string;
  status: "completed";
  outputDir: string;
  stages: Array<StageResult & { input?: unknown }>;
};
...
function tracePath(outputDir: string): string {
...
function collectFailedGates(stages: WorkflowResult["stages"]): WorkflowGateFailure[] {
  const failures: WorkflowGateFailure[] = [];
  for (const stage of stages) {
    const gate = stage.gate as CommandGateResult | undefined;
...
function mapStages(stages: WorkflowResult["stages"]): WorkflowStageStatus[] {
...
function deriveWorkflowStatus(stages: WorkflowStageStatus[]): WorkflowStatus["status"] {
...
export function inspectWorkflowTail(
  outputDir: string,
  ...
  return {
    outputDir,
    traceFile: filePath,
    events: lines.slice(-limit).map((line) => JSON.parse(line) as Record<string, unknown>),
  };
}
...
export function inspectWorkflowReport(outputDir: string): WorkflowReport {
  const status = inspectWorkflowStatus(outputDir);
  const finalStage = status.currentStage;
  const finalArtifact =
    finalStage?.outputFile
      ? {
          file: finalStage.outputFile,
          text: readArtifactPreview(finalStage.outputFile),
        }
      : undefined;
  const stageLines = status.stages.map((stage) =>
    `${stage.index}. ${stage.name} ${stage.status}${stage.sessionId ? ` session:${stage.sessionId}` : ""}`,
  );
  ...
  return {
    ...
    summary: [
      ...
      ...status.failedGates.map(
        (gate) => `Failed gate in ${gate.stage}: ${gate.command} exited ${gate.exitCode}.`,
      ),
    ].join("\n"),
  };
}

export function writeWorkflowStatus(outputDir: string): WorkflowStatus {
  const status = inspectWorkflowStatus(outputDir);
  fs.writeFileSync(path.join(outputDir, "status.json"), `${JSON.stringify(status, null, 2)}\n`);
  return status;
}

export function writeWorkflowStatusSnapshot(
  outputDir: string,
